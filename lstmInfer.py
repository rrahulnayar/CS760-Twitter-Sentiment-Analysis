...
with open('./data/train.pkl', 'rb') as train:
	...
	with tf.Session() as sess:
		#saver = tf.train.import_meta_graph('/home/cgarg/cs760/CS760-Twitter-Sentiment-Analysis/lstm_256u_2l_100b_sgd_0.05lr/checkpoints/sentiment.ckpt.meta')
		saver.restore(sess, "./lstm_256u_2l_100b_sgd_0.05lr/checkpoints/sentiment.ckpt")
		...
		if memoryNDtype == 'blstm':
			...
			for ii, (x, y) in enumerate(get_batches(test_x, test_y, batchSize), 1):
				...
				batch_acc, test_state, prec, recall, accr, sigPre = sess.run([accuracy, final_state, pre_op,rec_op, acc_op,predictions], feed_dict=feed)
				test_acc.append(batch_acc)
				for actVal in sigPre:
					tx = str(actVal).strip('[')
					tx = tx.strip(']')
